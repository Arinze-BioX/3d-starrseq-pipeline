...
def remove_table(table_name, db_name, user):
  try:
...
def load_data_to_postgres(tsv_file, table_name, db_name, user, column_names):
    """
...
def cross_join_tables(db_name, user, table1, table2, needed_columns, new_table, output_file):
  """
...
#cross_join_tables(db_name, table1, table2, needed_columns, new_table)

remove_table('combined_bin_pair', args.db_name, 
                      args.user)
start = time.time()
cross_join_tables(args.db_name, args.user, "frag1_table", 
